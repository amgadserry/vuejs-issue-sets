  ...
  export default {
    ...
    methods: {
      ...
      selectCurrent () {
        ...
        } else if (this.$store.state.Hierarchy.selectedNodes[this.$store.state.Hierarchy.selectedNodes.length - 1] !== this.data) {
          this.childrenOpacity = 0
          new TWEEN.Tween(this.childrenOpacity)
            .easing(TWEEN.Easing.Quadratic.Out)
    ...
    computed: {
      ...
      levelDiff () {
        return Math.abs(this.data.level - this.$store.state.Hierarchy.currentlySelectedLevel)
      },
      display () {
        if (this.isSelected) {
          return true
        }
        if (this.data.level > this.$store.state.Hierarchy.selectedNodes.length) {
          if (this.$store.state.Hierarchy.selectedNodes[this.$store.state.Hierarchy.selectedNodes.length - 1].employees.find(
              (el) => el === this.data)) {
            return true
          }
        }
      ...
      displayChildrenLines () {
        return this.$store.state.Hierarchy.selectedNodes[this.$store.state.Hierarchy.selectedNodes.length - 1] === this.data
      },
      x () {
      ...
      isSelected () {
        return this.$store.state.Hierarchy.selectedNodes.find((el) => this.data === el)
      }
    }
  }
</script>
